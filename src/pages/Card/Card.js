...
const Card = (match) => {
  const { data, getById, deleteItemById, toggleArchiveItem } = useContext(
    catalogContext
  )
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      timeout = setTimeout(
        () =>
          setLoading((prev) => (prev = "Произошла ошибка во время загрузки")),
        2000
      )
    }
    return () => {
  ...
  if (data && data.length === 1) {
    ...
    return (
      <>
        <div className={`card container pop-in ${archived ? "archived" : ""}`}>
          <div className="row">
              ...
              <>
                {admin ? (
                  <div className="admin">
                    ...
                    <div className="delete" style={{ marginTop: 5 }}>
                      <button
                        onClick={() => deleteItem()}
                ...
                <div className="img">
                  {imagesArray && imagesArray.length > 1 ? (
                    ...
                    <img src={imagesArray && imagesArray[0]} alt={text} />
                  )}
                </div>
                <div className="text-info">
                  ...
                  {!archived && <InCart id={id} inCard={true} />}
                </div>
              </>
            ) : (
              <div>
