...
const Gallery = ({ setImagesArray, imagesArray, loadImages, close }) => {
  ...
  return (
    <div
      className={`modal-wrapper ${drag || !gallery.length ? "dragging" : ""}`}
    ...
    >
          ...
          {gallery.map((img, idx) => (
            <div className="item pop-in" key={idx}>
              ...
              <img
                ...
                alt=""
                onClick={() => {
                  setImagesArray((prevArray) => prevArray.concat(img))
                  close()
                }}
              />
            </div>
      ...
      <div className="modal-controls">
        <label className="btn btn-success btn-expanded" htmlFor="loadFile">
          Загрузить файл&nbsp;&nbsp;&nbsp;&nbsp;
          <UploadIcon fill="var(--main)" height="1.4em" viewbox="0 0 20 24" />
        </label>
        <input
          className="hidden"
          type="file"
