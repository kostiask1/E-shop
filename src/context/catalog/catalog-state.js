...
export const CatalogState = ({ children }) => {
  ...
  const { filters, data, cart, storage, rowData, category, searchText } = state
  ...
  const checkStorage = (gotElements) => {
    if (storage.length) {
      if (rowData.length) {
        ...
        let itemsToDelete = storageClone.filter((item) => !data.includes(item))
        ...
        if (gotElements.length === storageClone.length) {
          let deletedItems = gotElements.length
            ? storageClone.filter((item, index) => !gotElements[index])
            : storageClone
