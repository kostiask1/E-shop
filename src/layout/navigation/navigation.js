...
const Navigation = () => {
  ...
  const clickMenuListener = (e) => {
    let inComponent = e.target.matches(`.drawer, .drawer *, .navbar__wrapper *`)
    if (!inComponent) {
      ...
      <div className="container">
        <div className="navbar__wrapper">
          <div className="menu-toggler" onClick={handleOpen}>
            <BarsIcon fill="var(--main-2)" width="3em" height="3em" />
          </div>
          ...
          <div
            className="nav-cart"
            onClick={() => cartRef.current && cartRef.current.open()}
            style={storage.length > 0 ? { right: 16 } : {}}
          >
            <CartIcon fill={`var(--${storage.length ? "main-2" : "gray"})`} />
            {cart.length > 0 && (
